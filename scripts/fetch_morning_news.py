...
"""
早朝簡報採集腳本
每日 06:00 自動運行，抓取全球新聞 RSS → data/morning_brief_YYYYMMDD.json
覆蓋: 政治 | 軍事 | 經濟 | AI大模型
"""
...
CATEGORY_KEYWORDS = {
    '軍事': ['war', 'military', 'troops', 'attack', 'missile', 'army', 'navy', 'weapons',
              '戰', '軍', '導彈', '士兵', 'ukraine', 'russia', 'china sea', 'nato'],
...
def _safe_parse_xml(xml_text, max_size=5*1024*1024):
    """安全解析 XML：限制大小，禁用外部實體（防 XXE）。"""
    if len(xml_text) > max_size:
        log.warning(f'XML 內容過大 ({len(xml_text)} bytes)，跳過')
        return None
    # 剝離 DOCTYPE / ENTITY 聲明以防 XXE
...
def main():
    ...
    parser.add_argument('--force', action='store_true', help='強制採集，忽略冪等鎖')
    ...
    if lock_file.exists() and not args.force:
        ...
        if age < 3600:  # 1小時內不重複
            log.info(f'今日已採集（{today}），跳過（使用 --force 強制採集）')
            return
    # 注意：lock 放到採集成功後再 touch，防止失敗也鎖定
    ...
    for cf in custom_feeds:
        ...
        if cat in enabled_cats and feed_url:
            ...
            if validate_url(feed_url):
                merged_feeds.setdefault(cat, []).append((cf.get('name', '自定義'), feed_url))
            else:
                log.warning(f'自定義源 URL 不合法，跳過: {feed_url}')

    log.info(f'開始採集 {today}...')
    log.info(f'  啓用分類: {", ".join(enabled_cats)}')
    if user_keywords:
        log.info(f'  關注詞: {", ".join(user_keywords)}')
    if custom_feeds:
        log.info(f'  自定義源: {len(custom_feeds)} 個')
    ...
    for category, feeds in merged_feeds.items():
        ...
        log.info(f'    {category}: {len(items)} 條')
    ...
    log.info(f'✅ 完成：共 {total} 條新聞 → {today_file.name}')
